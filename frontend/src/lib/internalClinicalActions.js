...
export async function closeAuthoritativeClinicalEpisode({
  api,
  session,
  episodeId,
  status,
  ...
  if (status !== 'RESPONDIDO' || !Array.isArray(responses) || responses.length === 0) {
    return { ok: false, errorCode: 'response_required' };
  }
  if (followUpPending) {
    return { ok: false, errorCode: 'follow_up_pending' };
  }
  if (handoffRequired && !handoffAcknowledged) {
    return { ok: false, errorCode: 'handoff_acknowledgement_missing' };
  }
  if (acknowledgementRequired && responses[responses.length - 1]?.status !== 'LEIDA') {
    return { ok: false, errorCode: 'acknowledgement_missing' };
  }
  if (responses.some((item) => item?.lateAfterDisposition && !item?.lateReviewedAt)) {
    return { ok: false, errorCode: 'late_response_review_pending' };
  }

  const payload = {
    follow_up_pending: false,
    acknowledgement_required: Boolean(acknowledgementRequired),
    handoff_required: Boolean(handoffRequired),
    handoff_acknowledged: Boolean(handoffRequired && handoffAcknowledged),
  };

  try {
    const episode = await api.closeEpisode(cleanEpisodeId, payload);
    return { ok: true, episode };
  } catch (error) {
    return { ok: false, errorCode: error?.code || 'operation_failed' };
  }
