 ...
 return <div data-testid="contacto-page">
  ...
  <section className="py-20 bg-white"><div className="max-w-7xl mx-auto px-6 grid lg:grid-cols-[.8fr_1.2fr] gap-12">
   ...
   <form onSubmit={submit} className="rounded-3xl border bg-slate-50 p-7 lg:p-9 shadow-sm"><h2 className="text-2xl font-bold text-slate-900">Solicitar información</h2><p className="text-slate-600 mt-2">Le responderemos para concretar la necesidad antes de preparar una propuesta.</p><div className="grid sm:grid-cols-2 gap-4 mt-7">
    <div><Label htmlFor="name">Nombre completo *</Label><Input id="name" name="name" autoComplete="name" maxLength={80} value={formData.name} onChange={e=>set('name',e.target.value)} required/></div>
    <div><Label htmlFor="company">Empresa u organización *</Label><Input id="company" name="organization" autoComplete="organization" maxLength={120} value={formData.company} onChange={e=>set('company',e.target.value)} required/></div>
    <div><Label htmlFor="email">Correo electrónico *</Label><Input id="email" name="email" autoComplete="email" type="email" maxLength={254} value={formData.email} onChange={e=>set('email',e.target.value)} required/></div>
    <div><Label htmlFor="phone">Teléfono</Label><Input id="phone" name="tel" autoComplete="tel" type="tel" maxLength={20} value={formData.phone} onChange={e=>set('phone',e.target.value)}/></div>
   ...
   <label className="flex items-start gap-3 mt-5 text-sm text-slate-600"><Checkbox id="privacy" checked={formData.accepts_privacy} onCheckedChange={v=>set('accepts_privacy',v===true)} className="mt-0.5"/><span><label htmlFor="privacy">Acepto la </label><Link to="/politica-privacidad" className="text-[#005EB8] hover:underline">política de protección de datos</Link><label htmlFor="privacy"> para que GASI responda a mi consulta. No enviaré información clínica o de salud por este formulario.</label></span></label>
