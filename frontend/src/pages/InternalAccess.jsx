...
export default function InternalAccess() {
  ...
  const { identities, session, lastError, signInSynthetic, signOut } = useInternalPrototypeAuth();
  ...
  const signIn = (identityId) => {
    if (signInSynthetic(identityId)) navigate('/interno/prototipo-clinico');
  };
    ...
    <main className="min-h-screen bg-slate-950 text-slate-100 py-10" data-testid="internal-access-prototype">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {activeIdentities.map((identity) => {
            ...
            return (
              <article key={identity.id} className="rounded-2xl border border-slate-800 bg-slate-900 p-5">
                ...
                <button type="button" onClick={() => signIn(identity.id)} className="mt-5 w-full rounded-lg bg-cyan-400 hover:bg-cyan-300 text-slate-950 font-semibold px-4 py-2">Simular acceso individual</button>
              </article>
            );
          })}
        </section>
        ...
        <section className="rounded-2xl border border-slate-800 bg-slate-900 p-6 mb-8">
          ...
          <p className="text-slate-400 mt-2">Una identidad revocada queda fuera del acceso. En producción, la revocación deberá invalidar sesiones/tokens del proveedor de identidad y quedar auditada.</p>
          <div className="mt-4 space-y-3">
            {revokedIdentities.map((identity) => (
              <div key={identity.id} className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 rounded-xl border border-rose-500/20 bg-rose-500/5 p-4">
                ...
                <button type="button" onClick={() => signIn(identity.id)} className="rounded-lg border border-rose-400/40 text-rose-200 px-3 py-2 text-sm">Probar bloqueo</button>
              </div>
            ))}
          </div>
        </section>
