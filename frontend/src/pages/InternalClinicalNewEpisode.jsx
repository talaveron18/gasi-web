...
export default function InternalClinicalNewEpisode() {
  ...
  const submit = async (event) => {
    ...
    setState('SAVED');
    setForm((current) => ({ ...EMPTY_FORM, center: current.center }));
  };
    ...
    <main className="min-h-screen bg-slate-950 text-slate-100 py-10" data-testid="authoritative-new-episode">
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <form onSubmit={submit} className="space-y-5 rounded-2xl border border-slate-800 bg-slate-900 p-5">
          ...
          {Number(form.level) === 1 && <div role="alert" className="rounded-lg border border-red-400/40 bg-red-500/10 p-4 text-sm text-red-100"><strong>Nivel 1:</strong> realizar llamada telefónica directa al facultativo. La web NO sustituye ni debe retrasar esa llamada.</div>}
          {Number(form.level) === 3 && <div className="rounded-lg border border-amber-400/40 bg-amber-400/10 p-4 text-sm text-amber-100">Gestión/documentación puede diseñarse, pero cualquier indicación médica remota, prescripción o actuación enfermera dependiente de validación jurídica permanece <strong>BLOQUEADA PARA ACTIVACIÓN REAL</strong>.</div>}
          ...
          <button type="submit" disabled={state === 'SAVING' || centers.length === 0} className="rounded-lg bg-cyan-300 px-5 py-3 text-sm font-semibold text-slate-950 disabled:opacity-50">{state === 'SAVING' ? 'Abriendo…' : 'Abrir caso sintético'}</button>
          ...
          {state === 'SAVED' && createdEpisode && (
            <div role="status" className="rounded-lg border border-emerald-400/30 bg-emerald-400/10 p-4 text-sm text-emerald-100">
              ...
              <div className="mt-3 flex flex-wrap gap-4"><Link to={`/interno/prototipo-clinico/caso/${encodeURIComponent(createdEpisode.id)}`} className="font-semibold underline">Abrir este caso autoritativo</Link><Link to="/interno/prototipo-clinico" className="underline">Volver al listado</Link></div>
            </div>
          )}
        </form>
      </div>
