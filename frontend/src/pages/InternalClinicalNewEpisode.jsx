  ...
  return (
    <main className="min-h-screen bg-slate-950 text-slate-100 py-10" data-testid="authoritative-new-episode">
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mb-6 flex flex-wrap items-center justify-between gap-3"><div><p className="text-sm text-cyan-300">Frente B · prototipo sintético</p><h1 className="mt-1 text-3xl font-bold">{roleCopy.title}</h1><p className="mt-2 text-sm text-slate-400">{roleCopy.label} · identidad individual y acceso limitado a centros asignados.</p></div><Link to="/interno/prototipo-clinico" className="rounded-lg border border-slate-700 px-4 py-2 text-sm">Volver a casos</Link></div>
        <div className="mb-5 rounded-xl border border-amber-400/30 bg-amber-400/10 p-4 text-sm text-amber-100">No usar datos reales. Este entorno permanece <strong>BLOQUEADO PARA ACTIVACIÓN REAL</strong> hasta cerrar infraestructura, privacidad y gates jurídicos aplicables.</div>
        <form onSubmit={submit} className="space-y-5 rounded-2xl border border-slate-800 bg-slate-900 p-5">
          <label className="block"><span className="text-sm font-semibold">Referencia de paciente sintética</span><input value={form.patientRef} onChange={(event) => update('patientRef', event.target.value)} required maxLength={120} placeholder="PAT-DEMO-001" className="mt-2 w-full rounded-lg border border-slate-700 bg-slate-950 p-3 text-sm" /><span className="mt-1 block text-xs text-slate-500">Usa únicamente identificadores inequívocamente ficticios.</span></label>
          ...
          <label className="block"><span className="text-sm font-semibold">Situación sintética</span><textarea value={form.summary} onChange={(event) => update('summary', event.target.value)} required rows={7} maxLength={4000} placeholder="Descripción clínica exclusivamente ficticia para probar el flujo." className="mt-2 w-full rounded-lg border border-slate-700 bg-slate-950 p-3 text-sm" /></label>
          <button type="submit" disabled={state === 'SAVING' || centers.length === 0} className="rounded-lg bg-cyan-300 px-5 py-3 text-sm font-semibold text-slate-950 disabled:opacity-50">{state === 'SAVING' ? 'Abriendo…' : `Abrir ${session.role === 'nurse' ? 'caso' : 'consulta'} sintética`}</button>
          ...
          {state === 'SAVED' && createdEpisode && <div role="status" className="rounded-lg border border-emerald-400/30 bg-emerald-400/10 p-4 text-sm text-emerald-100">Episodio creado por la autoridad sintética: <strong>{createdEpisode.id}</strong>. Estado {createdEpisode.status || 'ABIERTO'} · {createdEpisode.discipline || roleCopy.label}.<div className="mt-3 flex flex-wrap gap-4"><Link to={`/interno/prototipo-clinico/caso/${encodeURIComponent(createdEpisode.id)}`} className="font-semibold underline">Abrir este episodio</Link><Link to="/interno/prototipo-clinico" className="underline">Volver al listado</Link></div></div>}
        </form>
      </div>
    </main>
  );
