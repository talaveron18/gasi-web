    ...
    <main className="min-h-screen bg-slate-950 text-slate-100 py-10" data-testid="authoritative-new-episode">
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <form onSubmit={submit} className="space-y-5 rounded-2xl border border-slate-800 bg-slate-900 p-5">
          ...
          {roleCopy.level && Number(form.level) === 3 && <div className="rounded-lg border border-amber-400/40 bg-amber-400/10 p-4 text-sm text-amber-100">Nivel 3: gestión o documentación sin urgencia asistencial inmediata.</div>}
          {!roleCopy.level && <div className="rounded-lg border border-cyan-400/30 bg-cyan-400/10 p-4 text-sm text-cyan-100">Consulta propia de {roleCopy.label}, limitada a su disciplina profesional.</div>}
          ...
          {state === 'SAVED' && createdEpisode && <div role="status" className="rounded-lg border border-emerald-400/30 bg-emerald-400/10 p-4 text-sm text-emerald-100">Episodio creado por la sesión autenticada: <strong>{createdEpisode.id}</strong>. Estado {createdEpisode.status || 'ABIERTO'} · {createdEpisode.discipline || roleCopy.label}.<div className="mt-3 flex flex-wrap gap-4"><Link to={`/interno/clinica/caso/${encodeURIComponent(createdEpisode.id)}`} className="font-semibold underline">Abrir este episodio</Link><Link to="/interno/prototipo-clinico" className="underline">Volver al listado</Link></div></div>}
        </form>
      </div>
    </main>
  );
