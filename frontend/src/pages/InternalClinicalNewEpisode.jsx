...
export default function InternalClinicalNewEpisode(){
 ...
 const saveEpisode=async(confirmDistinctFromPatientId=null)=>{setState('SAVING');setErrorCode(null);setCreatedEpisode(null);setDuplicateCandidate(null);const result=await createAuthoritativeClinicalEpisode({api,session,patientId:selectedPatient&&selectedPatient.id||null,patient:patientPayload,confirmDistinctFromPatientId,center:form.center,level:roleCopy.level?form.level:2,summary:form.summary});if(!result.ok){setState('ERROR');setErrorCode(result.errorCode);if(result.candidate)setDuplicateCandidate(normalizeCandidate(result.candidate));return;}setCreatedEpisode(result.episode);setState('SAVED');setSelectedPatient(null);setSearchTerm('');setSearchResults([]);setNewPatientOpen(false);setForm(current=>Object.assign({},EMPTY_FORM,{center:current.center}));};
 const submit=async event=>{event.preventDefault();await saveEpisode(null);};
  ...
  <form onSubmit={submit} aria-busy={state==='SAVING'} className="space-y-5 rounded-2xl border border-slate-800 bg-slate-900 p-5">
   <section aria-labelledby="patient-heading"><h2 id="patient-heading" className="font-semibold">1. Identificar a la persona</h2><p className="mt-1 text-xs text-slate-500">Busca antes de crear una historia nueva. Puedes usar nombre, DNI, número de empleado o número de historia.</p>
    {selectedPatient?<div className="mt-3"><PatientCard patient={selectedPatient} selected/><button type="button" onClick={clearPatient} className="mt-2 inline-flex items-center gap-1 text-sm text-cyan-300"><X className="w-4"/>Cambiar persona</button></div>:<>
     <label className="mt-3 block"><span className="sr-only">Buscar paciente existente</span><div className="relative"><Search className="absolute left-3 top-3.5 h-4 w-4 text-slate-500"/><input value={searchTerm} onChange={e=>{setSearchTerm(e.target.value);setNewPatientOpen(false);}} placeholder="Ej. Javier Suárez, DNI, empleado o GASI-HC-…" className="w-full rounded-lg border border-slate-700 bg-slate-950 py-3 pl-10 pr-3 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-300"/></div></label>
     ...
     <button type="button" onClick={startNew} className="mt-3 inline-flex items-center gap-2 rounded-lg border border-slate-700 px-4 py-2 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-300"><UserPlus className="w-4"/>Registrar nueva persona</button>
    </>}
   </section>
   ...
   {duplicateCandidate&&<div role="alert" className="rounded-lg border border-amber-400/40 bg-amber-400/10 p-4 text-sm text-amber-100"><strong>Posible historia ya existente.</strong><p className="mt-1">No se ha creado nada. Selecciona la historia si es la misma persona. Si has comprobado que es otra persona distinta, puedes crear una historia nueva de forma explícita.</p><div className="mt-3"><PatientCard patient={duplicateCandidate} onSelect={choosePatient}/></div><button type="button" disabled={state==='SAVING'} onClick={()=>saveEpisode(duplicateCandidate.id)} className="mt-3 rounded-lg border border-amber-300 px-4 py-2 font-semibold focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber-200">He comprobado que es otra persona: crear historia distinta</button></div>}
   ...
   <button type="submit" disabled={state==='SAVING'||centers.length===0||!patientReady||(!selectedPatient&&!newIdentityReady)} className="rounded-lg bg-cyan-300 px-5 py-3 text-sm font-semibold text-slate-950 disabled:opacity-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-100">{state==='SAVING'?'Abriendo…':'Abrir '+(session.role==='nurse'?'caso':'consulta')}</button>
