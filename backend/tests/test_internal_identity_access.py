...
from backend.internal_identity_access import (
    IdentityAccessError,
    authenticate_synthetic_identity,
    build_identity_state,
    revoke_identity,
    session_can_access_center,
)
from backend.internal_worker_management import WorkerAccessError, build_worker_access

NOW = datetime(2026, 9, 17, 7, 0, tzinfo=timezone.utc)


def _worker(worker_id="DEMO-NURSE-01", centers=("DEMO-CENTER-A",)):
    return build_worker_access(worker_id=worker_id, role="nurse", centers=centers)


def _master():
    return build_worker_access(worker_id="DEMO-MASTER-ADMIN", role="admin", master_admin=True)


def test_assigned_center_requires_matching_valid_session():
    identity = build_identity_state(worker=_worker())
    session = authenticate_synthetic_identity(identity=identity, now=NOW)
    assert session_can_access_center(identity=identity, session=session, center="DEMO-CENTER-A", now=NOW)
    assert not session_can_access_center(identity=identity, session=session, center="DEMO-CENTER-B", now=NOW)


def test_session_from_another_identity_is_rejected():
    first = build_identity_state(worker=_worker())
    second = build_identity_state(worker=_worker(worker_id="DEMO-NURSE-02"))
    session = authenticate_synthetic_identity(identity=first, now=NOW)
    assert not session_can_access_center(identity=second, session=session, center="DEMO-CENTER-A", now=NOW)


def test_expired_session_cannot_reach_assigned_center():
    identity = build_identity_state(worker=_worker())
    session = authenticate_synthetic_identity(identity=identity, now=NOW, ttl_minutes=30)
    assert not session_can_access_center(identity=identity, session=session, center="DEMO-CENTER-A", now=NOW + timedelta(minutes=31))
...
def test_master_revocation_invalidates_existing_session():
    ...
    assert not session_can_access_center(identity=revoked, session=session, center="DEMO-CENTER-A", now=NOW)
