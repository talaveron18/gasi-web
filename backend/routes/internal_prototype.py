...
def _require_synthetic(value: str, field_name: str) -> str:
    ...
    if not normalized or not any(token in normalized.upper() for token in ("DEMO", "FICTICIO", "SYNTH")):
        raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail=f"{field_name}_must_be_synthetic")
    return normalized
...
def _audit(actor: Actor, action: str, episode_id: Optional[str] = None, metadata: Optional[dict] = None) -> None:
    AUDIT.append({"at": _now(), "actor_id": actor.id, "actor_role": actor.role, "action": action, "episode_id": episode_id, "metadata": metadata or {}})
...
def change_level(episode_id: str, payload: LevelChangeInput, x_demo_actor_id: Optional[str] = Header(default=None)):
    ...
    episode["level_history"].append({"at": at, "actor_id": actor.id, "from": previous, "to": payload.level})
...
def record_disposition(episode_id: str, payload: DispositionInput, x_demo_actor_id: Optional[str] = Header(default=None)):
    ...
    if episode["status"] == "CERRADO":
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="closed_episode_immutable")
    event = {"id": f"{episode_id}-D{len(episode['disposition_events']) + 1}", "kind": payload.kind, "occurred_at": payload.occurred_at, "recorded_at": _now(), "recorded_by_id": actor.id, "recorded_by_role": actor.role}
...
def add_addendum(episode_id: str, payload: AddendumInput, x_demo_actor_id: Optional[str] = Header(default=None)):
    ...
    if episode["status"] == "CERRADO":
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="closed_episode_immutable")
    at = _now()
    addendum = {"id": f"{episode_id}-A{len(episode['addenda']) + 1}", "text": payload.text.strip(), "author_id": actor.id, "author_role": actor.role, "created_at": at}
...
def close_episode(episode_id: str, payload: CloseInput, x_demo_actor_id: Optional[str] = Header(default=None)):
    ...
    _audit(actor, "EPISODE_CLOSED", episode_id, {"acknowledgement_required": payload.acknowledgement_required, "handoff_required": payload.handoff_required, "handoff_acknowledged": payload.handoff_acknowledged})
    return episode
