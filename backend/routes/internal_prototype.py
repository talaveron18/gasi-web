...
ACTORS={'GASI-NURSE-01':Actor(id='GASI-NURSE-01',role='nurse',display_name='Enfermería GASI',centers=['Centro GASI Madrid 01']),'GASI-PHYS-01':Actor(id='GASI-PHYS-01',role='physician',display_name='Facultativo GASI',centers=['Centro GASI Madrid 01']),'GASI-PSY-01':Actor(id='GASI-PSY-01',role='psychologist',display_name='Psicología GASI',centers=['Centro GASI Madrid 01']),'GASI-PHYSIO-01':Actor(id='GASI-PHYSIO-01',role='physiotherapist',display_name='Fisioterapia GASI',centers=['Centro GASI Madrid 01']),'GASI-MASTER-01':Actor(id='GASI-MASTER-01',role='admin',display_name='Administración maestra GASI',centers=[])}
MASTER_ADMIN_ID='GASI-MASTER-01';EPISODES:Dict[str,dict]={};AUDIT=ValidatedAuditStream();DELEGATED_PRIVILEGES:Dict[str,set]={}
def _actor(i):
 a=ACTORS.get(i or '')
...
def _can_write(a,e):
 if a.role in ('nurse','physician'):return e['center'] in a.centers
 return a.role in ('psychologist','physiotherapist') and e['center'] in a.centers and e.get('discipline')==ROLE_DISCIPLINE[a.role]
...
@router.get('/health')
def health():return{'enabled':True,'service':'internal_clinical'}
@router.get('/workers')
def workers(x_actor_id:Optional[str]=Header(default=None)):
 a=_actor(x_actor_id)
...
def grant(worker_id:str,p:PrivilegeGrantInput,x_actor_id:Optional[str]=Header(default=None)):
 a=_actor(x_actor_id)
...
def revoke(worker_id:str,p:PrivilegeGrantInput,x_actor_id:Optional[str]=Header(default=None)):
 a=_actor(x_actor_id)
 if a.id!=MASTER_ADMIN_ID:raise HTTPException(403,'master_account_only')
...
def create(p:CreateEpisode,x_actor_id:Optional[str]=Header(default=None)):
 a=_actor(x_actor_id)
 ...
 i=f'GASI-EP-{len(EPISODES)+1:06d}';n=_now();e={'id':i,'patient_ref':patient,'center':center,'discipline':ROLE_DISCIPLINE[a.role],'level':p.level,'status':'ABIERTO','summary':p.summary.strip(),'created_at':n,'created_by_id':a.id,'created_by_role':a.role,'responses':[],'addenda':[],'level_history':[],'disposition_events':[],'closed_at':None,'closed_by_id':None};EPISODES[i]=e;_audit(a,'EPISODE_CREATED',i,{'level':p.level,'discipline':e['discipline']});return e
@router.get('/episodes')
def episodes(x_actor_id:Optional[str]=Header(default=None)):
 a=_actor(x_actor_id);v=[e for e in EPISODES.values()if _can(a,e)];_audit(a,'EPISODES_LIST_VIEWED',metadata={'count':len(v)});return[_admin(e)for e in v]if a.role=='admin'else v
@router.get('/episodes/{episode_id}')
def get(episode_id:str,x_actor_id:Optional[str]=Header(default=None)):
 a=_actor(x_actor_id);e=_ep(episode_id);_access(a,e);_audit(a,'EPISODE_VIEWED',episode_id);return _admin(e)if a.role=='admin'else e
@router.post('/episodes/{episode_id}/privileged-access')
def privileged(episode_id:str,p:PrivilegedAccessInput,x_actor_id:Optional[str]=Header(default=None)):
 a=_actor(x_actor_id)
 if not _has(a,'clinical_record_privileged_read'):raise HTTPException(403,'privileged_record_access_required')
 e=_ep(episode_id);ref=_required(p.reference,'reference');_audit(a,'PRIVILEGED_CLINICAL_RECORD_ACCESSED',episode_id,{'reason':p.reason,'reference':ref,'delegated':a.id!=MASTER_ADMIN_ID});return{**e,'privileged_access':{'reason':p.reason,'reference':ref,'accessed_at':_now(),'accessed_by_id':a.id,'delegated':a.id!=MASTER_ADMIN_ID},'read_only':True}
@router.post('/episodes/{episode_id}/level')
def level(episode_id:str,p:LevelChangeInput,x_actor_id:Optional[str]=Header(default=None)):
 a=_actor(x_actor_id)
 if a.role!='nurse':raise HTTPException(403,'nurse_only')
 e=_ep(episode_id);_write_access(a,e)
...
def disposition(episode_id:str,p:DispositionInput,x_actor_id:Optional[str]=Header(default=None)):
 a=_actor(x_actor_id)
...
def respond(episode_id:str,p:ResponseInput,x_actor_id:Optional[str]=Header(default=None)):
 a=_actor(x_actor_id)
...
def late_review(episode_id:str,response_id:str,x_actor_id:Optional[str]=Header(default=None)):
 a=_actor(x_actor_id)
 if a.role!='physician':raise HTTPException(403,'physician_only')
 e=_ep(episode_id);_write_access(a,e);r=next((x for x in e['responses']if x['id']==response_id),None)
...
def delivery(episode_id:str,response_id:str,p:EvidenceInput,x_actor_id:Optional[str]=Header(default=None)):
 a=_actor(x_actor_id);e=_ep(episode_id);_write_access(a,e)
 if a.role not in CLINICAL_ROLES:raise HTTPException(403,'clinical_role_required')
...
def correct_response(episode_id:str,entry_id:str,p:CorrectionInput,x_actor_id:Optional[str]=Header(default=None)):
 a=_actor(x_actor_id);e=_ep(episode_id);_write_access(a,e);return _correct(a,e,_entry(a,e,'responses',entry_id),p,'response')
@router.post('/episodes/{episode_id}/addenda')
def addendum(episode_id:str,p:AddendumInput,x_actor_id:Optional[str]=Header(default=None)):
 a=_actor(x_actor_id)
 if a.role not in CLINICAL_ROLES:raise HTTPException(403,'clinical_role_required')
 e=_ep(episode_id);_write_access(a,e)
 if e['status']=='CERRADO':raise HTTPException(409,'closed_episode_immutable')
...
def correct_addendum(episode_id:str,entry_id:str,p:CorrectionInput,x_actor_id:Optional[str]=Header(default=None)):
 a=_actor(x_actor_id);e=_ep(episode_id);_write_access(a,e);return _correct(a,e,_entry(a,e,'addenda',entry_id),p,'addendum')
@router.post('/episodes/{episode_id}/close')
def close(episode_id:str,p:CloseInput,x_actor_id:Optional[str]=Header(default=None)):
 a=_actor(x_actor_id)
 if a.role not in CLINICAL_ROLES:raise HTTPException(403,'clinical_role_required')
 e=_ep(episode_id);_write_access(a,e)
...
@router.get('/audit')
def audit(x_actor_id:Optional[str]=Header(default=None)):
 a=_actor(x_actor_id)
 if a.id!=MASTER_ADMIN_ID:raise HTTPException(403,'master_account_only')
 return AUDIT