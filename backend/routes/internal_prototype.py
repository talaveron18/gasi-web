...
async def _actor(i):
 a=await _store().get_worker(i or '')
...
@router.get('/workers')
async def workers(x_actor_id:Optional[str]=Header(default=None)):
 a=await _actor(x_actor_id)
...
async def grant(worker_id:str,p:PrivilegeGrantInput,x_actor_id:Optional[str]=Header(default=None)):
 a=await _actor(x_actor_id)
...
async def revoke(worker_id:str,p:PrivilegeGrantInput,x_actor_id:Optional[str]=Header(default=None)):
 a=await _actor(x_actor_id)
 if a['id']!=MASTER_ADMIN_ID:raise HTTPException(403,'master_account_only')
...
async def create(p:CreateEpisode,x_actor_id:Optional[str]=Header(default=None)):
 a=await _actor(x_actor_id)
...
@router.get('/episodes')
async def episodes(x_actor_id:Optional[str]=Header(default=None)):
 a=await _actor(x_actor_id);query={}if a['role']=='admin'else{'center':{'$in':a.get('centers',[])}};v=await _store().list_episodes(query);await _audit(a,'EPISODES_LIST_VIEWED',metadata={'count':len(v)});return[_admin(e)for e in v]if a['role']=='admin'else v
@router.get('/episodes/{episode_id}')
async def get(episode_id:str,x_actor_id:Optional[str]=Header(default=None)):
 a=await _actor(x_actor_id);e=await _ep(episode_id);_access(a,e);await _audit(a,'EPISODE_VIEWED',episode_id);return _admin(e)if a['role']=='admin'else e
@router.post('/episodes/{episode_id}/privileged-access')
async def privileged(episode_id:str,p:PrivilegedAccessInput,x_actor_id:Optional[str]=Header(default=None)):
 a=await _actor(x_actor_id)
...
async def level(episode_id:str,p:LevelChangeInput,x_actor_id:Optional[str]=Header(default=None)):
 a=await _actor(x_actor_id)
...
async def disposition(episode_id:str,p:DispositionInput,x_actor_id:Optional[str]=Header(default=None)):
 a=await _actor(x_actor_id);e=await _ep(episode_id);_write_access(a,e)
 if e['status']=='CERRADO':raise HTTPException(409,'closed_episode_immutable')
...
async def respond(episode_id:str,p:ResponseInput,x_actor_id:Optional[str]=Header(default=None)):
 a=await _actor(x_actor_id)
...
async def late_review(episode_id:str,response_id:str,x_actor_id:Optional[str]=Header(default=None)):
 a=await _actor(x_actor_id)
 if a['role']!='physician':raise HTTPException(403,'physician_only')
...
async def delivery(episode_id:str,response_id:str,p:EvidenceInput,x_actor_id:Optional[str]=Header(default=None)):
 a=await _actor(x_actor_id);e=await _ep(episode_id);_write_access(a,e);r=next((x for x in e['responses']if x['id']==response_id),None)
 if not r:raise HTTPException(404,'response_not_found')
...
async def correct_response(episode_id:str,entry_id:str,p:CorrectionInput,x_actor_id:Optional[str]=Header(default=None)):
 a=await _actor(x_actor_id);e=await _ep(episode_id);_write_access(a,e);return await _correct(a,e,_entry(a,e,'responses',entry_id),p,'response')
@router.post('/episodes/{episode_id}/addenda')
async def addendum(episode_id:str,p:AddendumInput,x_actor_id:Optional[str]=Header(default=None)):
 a=await _actor(x_actor_id);e=await _ep(episode_id);_write_access(a,e)
 if e['status']=='CERRADO':raise HTTPException(409,'closed_episode_immutable')
...
async def correct_addendum(episode_id:str,entry_id:str,p:CorrectionInput,x_actor_id:Optional[str]=Header(default=None)):
 a=await _actor(x_actor_id);e=await _ep(episode_id);_write_access(a,e);return await _correct(a,e,_entry(a,e,'addenda',entry_id),p,'addendum')
@router.post('/episodes/{episode_id}/close')
async def close(episode_id:str,p:CloseInput,x_actor_id:Optional[str]=Header(default=None)):
 a=await _actor(x_actor_id);e=await _ep(episode_id);_write_access(a,e)
...
@router.get('/audit')
async def audit(x_actor_id:Optional[str]=Header(default=None)):
 a=await _actor(x_actor_id)
 if a['id']!=MASTER_ADMIN_ID:raise HTTPException(403,'master_account_only')
